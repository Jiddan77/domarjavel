...
def main():
    ...
    print(f"🔎 Backfill penalty-sträng {args.season}: {len(t)} matcher.")
    changed = 0
    for i, m in enumerate(t, 1):
        ...
        pen = to_pair_str(sts.get("homeTeamGoalsFromPenalties"), sts.get("visitingTeamGoalsFromPenalties"))
        if m.get("penalty") != pen:
            m["penalty"] = pen; changed += 1
            print(f"✅ [{i}/{len(t)}] {mid}: penalty → {pen}")
        else:
